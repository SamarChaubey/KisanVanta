...
const updates = [
  ['Minimum Support Price notice', 'Government circular Â· 08 Sep 2026', 'Review updated crop prices before confirming today\'s lots.'],
  ['e-Mandi operating hours', 'Department update Â· 07 Sep 2026', 'Extended receiving hours apply to selected centres.'],
  ['Quality grading advisory', 'Food procurement board Â· 05 Sep 2026', 'Use the revised moisture checklist at intake.'],
];
...
function Lookup({ requests }) { const [value, setValue] = useState(''); const result = requests.find(item => JSON.stringify(item).toLowerCase().includes(value.toLowerCase()) && value); return <section className="admin-panel lookup-panel"><span className="admin-kicker">Gate and help desk</span><h2>QR / ID lookup</h2><p className="admin-help">Scan a gate pass with a QR reader or enter a request, farmer or lot ID.</p><div className="lookup-box"><input value={value} onChange={event => setValue(event.target.value)} placeholder="Enter ID or scan code" /><button className="admin-primary">Search record</button></div>{result ? <div className="lookup-result"><Status tone="green">Record found</Status><h3>Request {String(result._id).slice(-8)}</h3><p>Farmer ID: {result.farmerId} Â· Centre ID: {result.centreId}</p><p>Status: <b>{result.status}</b> Â· Preferred: {result.requestedDate} at {result.preferredTime}</p></div> : <div className="lookup-placeholder">Lookup results will appear here.</div>}</section>; }
...
function Quality() { return <div className="admin-two-column"><section className="admin-panel"><span className="admin-kicker">Quality desk</span><h2>Quality checks</h2><div className="admin-metrics compact"><Metric label="Awaiting check" value="91" note="lots" tone="amber" /><Metric label="Accepted today" value="64" note="lots" tone="green" /><Metric label="Held for review" value="7" note="lots" tone="red" /></div><div className="check-row"><b>Wheat Â· Lot KVF102537</b><Status tone="amber">Moisture review</Status></div><div className="check-row"><b>Mustard Â· Lot KVF102529</b><Status tone="green">Grade A</Status></div></section><section className="admin-panel"><span className="admin-kicker">Weighbridge</span><h2>Weighing desk</h2><div className="weigh-card"><b>4 of 5 weighbridges active</b><strong>64 lots</strong><small>Current queue Â· estimated wait 42 min</small><button className="admin-outline">View weighbridge status</button></div></section></div>; }
function Storage({ risks }) { return <div className="admin-two-column"><section className="admin-panel"><span className="admin-kicker">Post-procurement</span><h2>Storage and lifting</h2>{risks.slice(0, 6).map(risk => <div className="storage-row" key={risk.centreId}><span><b>{risk.centreName}</b><small>{risk.liftingGap ? `${risk.liftingGap} bags/day lifting gap` : 'Lifting on plan'}</small></span><strong>{risk.storageUtilization || 0}%</strong><div className="storage-bar"><i style={{ width: `${risk.storageUtilization || 0}%` }} /></div></div>)}</section><section className="admin-panel"><span className="admin-kicker">Action queue</span><h2>Recommended actions</h2><div className="action-item"><b>Divert arrivals from Amer</b><small>Owner: Centre manager Â· Priority: immediate</small><button className="admin-outline">Escalate</button></div><div className="action-item"><b>Confirm evening dispatch</b><small>Owner: Logistics lead Â· Priority: today</small><button className="admin-outline">Assign</button></div></section></div>; }
